...
Base mode (`glyph_key=None`) remains zero-secret and renders visual lineage
for anomaly detection. Authenticated mode uses HMAC-SHA-256 with the domain
separator `AIML-GLYPH-V1` and binds the vault ID, record ID, sequence, entry
hash, previous Glyph MAC, and tri-timestamp. The HMAC-derived glyph is stored
with `glyph_mode`, `glyph_key_id`, `previous_glyph_mac`, and `glyph_mac`.

Keys are runtime-only: they are never written to the ledger, Vault, logs,
receipts, or exports. Supply `glyph_key` plus `glyph_key_id` to the layer, or
use the service environment variables `AIMS_GLYPH_KEY` and
`AIMS_GLYPH_KEY_ID`. Keep prior keys in `glyph_keys` after rotation;
`rotate_glyph_key()` records an immutable system event before switching keys.
...
from memory_core import AIMSMemorySystem, EntryType

memory = AIMSMemorySystem(Path("./data"))
